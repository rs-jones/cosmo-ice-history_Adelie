...
function core_plot = plot_core_concs(sample_data,plot_ts,x_lim,y_lim)
  ...
  has10 = false; has26 = false; has14 = false;
  ...
  for a = 1:length(sample_data.s)
      ...
      if this_s.nuclide10 == 1
          ...
          yy = [max(this_s.bottom_z) min(this_s.top_z)];
          plot(xx/1000,yy,'Color',l_col);
          hold on;
          ...
          for b = 1:length(this_s.top_z)
              yy = [this_s.top_z(b) this_s.bottom_z(b) this_s.bottom_z(b) this_s.top_z(b) this_s.top_z(b)];
              plot(xx/1000,yy,'Color',d_col);
          end
      end

      if isfield(this_s,'nuclide26')
      ...
      if isfield(this_s,'nuclide14')
          if this_s.nuclide14 == 1
              has14 = true;
              d_col = d_colours(1,:);
              l_col = l_colours(1,:);

              % Plot mean
              xx = [this_s.N14 this_s.N14];
              yy = [max(this_s.bottom_z) min(this_s.top_z)];
              plot(xx/1000,yy,'Color',l_col);
              hold on;

              % Plot 1-sigma box
              N14_low = this_s.N14 - this_s.dN14;
              N14_upp = this_s.N14 + this_s.dN14;
              xx = [N14_low N14_low N14_upp N14_upp N14_low];
              for b = 1:length(this_s.top_z)
                  yy = [this_s.top_z(b) this_s.bottom_z(b) this_s.bottom_z(b) this_s.top_z(b) this_s.top_z(b)];
                  plot(xx/1000,yy,'Color',d_col);
              end
  ...
  a1.YDir = 'reverse';
  xlabel('Nuclide concentration (k atoms g^{-1})');
  ylabel('Depth below surface (cm)');
  grid on;
  a1.GridLineStyle = ':';

  % Add key for nuclides
  keyItems = {};
  keyCols  = [];
  if has10
      keyItems{end+1} = '^{10}Be';
      keyCols(end+1,:) = d_colours(2,:);
  end
  if has14
      keyItems{end+1} = '^{14}C';
      keyCols(end+1,:) = d_colours(1,:);
  end
  if has26
      keyItems{end+1} = '^{26}Al';
      keyCols(end+1,:) = d_colours(5,:);
  end

  % Position in axes-normalized coordinates
  x0 = 0.95;   % right
  y0 = 0.03;   % near bottom
  dy = 0.07;   % vertical spacing

  for i = 1:numel(keyItems)
      text(a1, x0, y0 + (numel(keyItems)-i)*dy, keyItems{i}, ...
          'Units','normalized', ...
